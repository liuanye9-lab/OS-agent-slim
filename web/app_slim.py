...
def create_slim_app() -> FastAPI:
    ...
    # ------------------------------------------------------------------
    # Skill Routes (V12 SkillOS)
    # ------------------------------------------------------------------
    from web.routes.skills import register_skill_routes
    register_skill_routes(app)
    logger.info("Skill routes registered (SkillOS)")
    ...
    # ------------------------------------------------------------------
    skill_template = os.path.join(templates_dir, "skill_dashboard.html")
    ...
    async def skill_dashboard():
        if os.path.exists(skill_template):
            with open(skill_template, "r", encoding="utf-8") as f:
                return HTMLResponse(content=f.read())
        return HTMLResponse(content="<h1>Skill Dashboard template not found</h1>", status_code=404)
